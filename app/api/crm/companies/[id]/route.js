...
export async function GET(req, { params }) {
  try {
    ...
    const [company, contacts, deals, tasks, activities, meetings] = await Promise.all([
      ...
      Activity.find({ company: id }).populate("createdBy", "name email").sort({ createdAt: -1 }).limit(50).lean(),
    ...
    return NextResponse.json({ company, contacts, deals, tasks, activities, meetings });
  } catch (err) {
    console.error(err);
