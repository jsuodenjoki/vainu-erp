...
export default function CreateCompanyModal({ onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.name.trim()) { setError(t("crm.companies.fields.companyName") + " is required"); return; }
    setLoading(true);
    setError("");
    try {
  ...
  return (
    <ModalBase title={t("crm.companies.create.title")} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t("crm.companies.fields.companyName")} *
          </label>
          <input
            name="name"
            value={form.name}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t("crm.companies.fields.lifecycleStage")}
            </label>
            <select
              name="lifecycleStage"
          ...
          <div>
            ...
            >
              ...
              {SIZES.map((s) => (
                <option key={s} value={s}>{t(`crm.companySize.${s}`)}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t("crm.companies.fields.industry")}
          </label>
          <input
            name="industry"
            value={form.industry}
